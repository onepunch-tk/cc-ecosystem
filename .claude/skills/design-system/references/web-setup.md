...
Map `tokens.json` values to Tailwind v4 `@theme` variables in the global CSS file:
...
### DTCG Token Conversion

Extract `$value` from each DTCG token and map to `@theme` variables:

```
tokens.json                          →  @theme CSS
color.primary.$value: "#007AFF"      →  --color-primary: #007AFF;
spacing.md.$value: "16px"            →  (use --spacing: 4px base + scale)
border-radius.lg.$value: "12px"      →  --radius-lg: 12px;
breakpoint.md.$value: "768px"        →  --breakpoint-md: 768px;
```
...
- [ ] `@theme` variables match `tokens.json` values
