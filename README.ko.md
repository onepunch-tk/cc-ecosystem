...
└── .claude/
    ...
    ├── pipeline-state.json            # 파이프라인 Phase 상태 (ABAC)
...
| `harness-pipeline` | `/harness-pipeline` | 통합 개발 파이프라인 — Sequential (1-5파일) 또는 Team (6+파일) 모드 자동 감지 |
...
| `protect-files.sh` | PreToolUse (Edit/Write) | `bun.lock` 등 보호 대상 파일 수정 차단 |
...
| `abac-phase-policy.sh` | PreToolUse (Edit/Write) | Phase 기반 소스코드 차단 — plan phase에서 코드 수정 방지 |
